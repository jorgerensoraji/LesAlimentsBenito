...
function buildOrderSheetContent(order, settings = {}, logoImg = null) {
  ...
  // ══════════════════════════════════════════════════════════
  const tTop   = custBot;
  const tBot   = 86;   // raised 28 pt to make room for Notes row above footer
  const rowH   = 18;
  ...
  txt(cmd, 'NOTES / REMARQUES :', L + 4, noteBot + 18, 8, { bold: true });
  line(cmd, L + 110, noteBot + 15, R - 4, noteBot + 15, 0.4);
  const noteText = trunc(order.customer.message || '', 90);
  if (noteText) txt(cmd, noteText, L + 114, noteBot + 18, 8);
  line(cmd, L + 4, noteBot + 6, R - 4, noteBot + 6, 0.4);
