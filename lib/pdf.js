...
function buildOrderSheetContent(order, settings = {}, logoImg = null) {
  ...
  // ══════════════════════════════════════════════════════════
  // DATE ROW  (y: 670 → 692) — left: order date, right: delivery date
  // ══════════════════════════════════════════════════════════
  const dateTop = hBot;
  const dateBot = dateTop - 22;
  const dateMid = L + Math.round(W / 2);
  pdfRect(cmd, L, dateBot, W, 22, 0.8);
  line(cmd, dateMid, dateBot, dateMid, dateTop, 0.5);

  txt(cmd, 'DATE :', L + 6, dateBot + 7, 9, { bold: true });
  line(cmd, L + 48, dateBot + 3, dateMid - 6, dateBot + 3, 0.5);
  txt(cmd, date, L + 52, dateBot + 7, 9);

  txt(cmd, 'DATE DE LIVRAISON /', dateMid + 4, dateBot + 14, 6, { bold: true });
  txt(cmd, 'DELIVERY DATE :',     dateMid + 4, dateBot +  7, 6);
  const delivDate = order.customer.deliveryDate
    ? new Date(order.customer.deliveryDate + 'T12:00:00').toLocaleDateString('fr-CA')
    : '';
  line(cmd, dateMid + 100, dateBot + 3, R - 6, dateBot + 3, 0.5);
  if (delivDate) txt(cmd, delivDate, dateMid + 104, dateBot + 7, 9);
  ...
  const tTop    = custBot;
  const rowH    = 18;
  const noteH   = 36;                          // compact notes section height
  const tBot    = 58 + noteH;                  // = 94 — leaves room for notes above footer
  const maxRows = Math.floor((tTop - tBot - rowH) / rowH); // ~25 rows
  ...
  (order.items || []).slice(0, maxRows).forEach((item, i) => {
    ...
    const code = item.sku ? trunc(item.sku, 7) : String(i + 1).padStart(3, '0');
    ctr(cmd, code, cols[0], cols[1], y, 7);
    ctr(cmd, qtyLabel,                       cols[1], cols[2], y, 9);
    txt(cmd, name, cols[2] + 4, y, 9, { bold: true });
    // POIDS/WEIGHT column intentionally left blank — filled manually after delivery
  });
  ...
  // ══════════════════════════════════════════════════════════
  // NOTES SECTION  (tBot → 58) — fills all space above footer
  // ══════════════════════════════════════════════════════════
  const noteTop = tBot;
  const noteBot = 58;
  pdfRect(cmd, L, noteBot, W, noteTop - noteBot, 0.8);

  // Label row (top of notes box)
  txt(cmd, 'NOTES / REMARQUES :', L + 4, noteTop - 12, 8, { bold: true });
  line(cmd, L + 114, noteTop - 15, R - 4, noteTop - 15, 0.4);
  const noteText = trunc(order.customer.message || '', 95);
  if (noteText) txt(cmd, noteText, L + 118, noteTop - 12, 8);

  // Second underline — blank line for manual additions
  line(cmd, L + 4, noteTop - 26, R - 4, noteTop - 26, 0.4);

  // ══════════════════════════════════════════════════════════
  // FOOTER  (y: 10 → 58) — all positions relative to noteBot
  // ══════════════════════════════════════════════════════════
  line(cmd, L, noteBot, R, noteBot, 1.4);
  txt(cmd, "Nous n'acceptons pas de reclamation apres votre signature.", L, noteBot - 13, 7);
  txt(cmd, 'We do not accept any claim after your signature.',           L, noteBot - 23, 7);
  txt(cmd, 'CONDITIONS: NET 7 JOURS  *  TERMS NET 7 DAYS',              L, noteBot - 33, 7);
  ctr(cmd, "SIGNATURE DE L'ACHETEUR", 215, 435, noteBot - 12, 8, { bold: true });
  ctr(cmd, "BUYER'S SIGNATURE",       215, 435, noteBot - 22, 7);
  line(cmd, 235, noteBot - 38, 385, noteBot - 38, 0.5);
  pdfRect(cmd, R - 82, noteBot - 42, 82, 40, 1.2);
  ctr(cmd, 'TOTAL', R - 82, R, noteBot - 18, 12, { bold: true });
