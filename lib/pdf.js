...
function buildOrderSheetContent(order, settings = {}, logoImg = null) {
  ...
  const maxRows = 20;
  const rowH    = 18;
  const tBot    = tTop - (maxRows + 1) * rowH; // exact bottom for 20 item rows + header
  ...
  // Label + underline on first text row
  txt(cmd, 'NOTES / REMARQUES :', L + 4, noteTop - 12, 8, { bold: true });
  line(cmd, L + 114, noteTop - 15, R - 4, noteTop - 15, 0.4);

  // Wrap message across up to 4 lines inside the notes box
  const noteLines = wrapText(order.customer.message || '', 100).slice(0, 4);
  noteLines.forEach((ln, idx) => {
    txt(cmd, ln, L + 6, noteTop - 12 - (idx + 1) * 14, 8);
  });
