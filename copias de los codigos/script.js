...
async function generarYEnviarPDF() {
  ...
  const fechaStr = fecha.toISOString().replace(/[:.-]/g, "").slice(0, 15);
  const nombreOrden = nombre.replace(/\s+/g, '') + "_" + fechaStr;
  ...
  doc.setFontSize(18);
  doc.text(`Orden de Compra: ${nombreOrden}`, 10, 40);

  doc.setFontSize(14);
  doc.text(`Nombre del cliente: ${nombre}`, 10, 50);
  doc.setFontSize(12);
  doc.text(`Correo: ${email}`, 10, 58);
